...
* **Logging:** Use `dart:developer` `log` instead of `print` or app_log.dart.
...
## Routing (AutoRoute)
Use `auto_route` for all navigation needs (deep linking, web). Ensure users are redirected to login when unauthorized.
